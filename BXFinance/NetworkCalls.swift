...
struct NetworkCalls {
    static func getRiskEvaluation(username: String, riskData: String) async throws -> String {
        guard let url = URL(string: "\(K.apiBaseUrl)/riskEvaluation") else {
            fatalError("Could not initialize url: \(K.apiBaseUrl)/riskEvaluation")
        }
    ...
    static func getDeviceIpAddress() async throws -> String {
        guard let url = URL(string: K.ipAddressApiUrl) else {
            fatalError("Could not create url from \(K.ipAddressApiUrl)")
        }
